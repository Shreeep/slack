...
def user_profile_upload_photo(token, img_url, x_start, y_start, x_end, y_end):
    ...
    # get img and open it
    urllib.request.urlretrieve(img_url, f'{u_id}.jpg')
    img = Image.open(f'{u_id}.jpg')
    ...
    if os.path.exists('profile_imgs') is False:
        os.mkdir('profile_imgs')


    cropped.save(f'profile_imgs/{u_id}.jpg')
...
# token = ret['token']
# print(token)
# user_profile_upload_photo(token, 'https://www.courant.com/resizer/D9qmAnzR8PY5q-GBdUBBVuNVUTs=/415x311/top/arc-anglerfish-arc2-prod-tronc.s3.amazonaws.com/public/NTWCZKYTDJBI7CASRJ32F2RN6E.jpg', 0, 0, 415, 311)
